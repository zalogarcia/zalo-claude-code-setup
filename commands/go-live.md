...
- When the runbook came from an `/autopilot` run and EVERY surface is ACTIVATED with its proof signal, record the pass from the run's worktree in two places: its state file (`terminal_state` from `code_complete_not_live_verified` to `complete`, plus when), and a log in the repo's shared git directory, which outlives the worktree (`.autopilot/` lives in the worktree, and the user may `git worktree remove` the worktree after merging):
