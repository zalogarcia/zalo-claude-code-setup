...
# CRITICAL: never call bare `whisper-cli` — PATH resolves it to the Intel/Rosetta
# build in /usr/local/bin (CPU-only, ~hours for 1h of audio). The arm64+Metal build
# lives in /opt/homebrew/bin (~minutes on this M3 Max). Resolve explicitly:
WHISPER_BIN="/opt/homebrew/bin/whisper-cli"
if [ ! -x "$WHISPER_BIN" ]; then WHISPER_BIN="$(command -v whisper-cli)"; fi
