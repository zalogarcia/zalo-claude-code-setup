...
23. **Multi-file edits require `/qa-loop`.** Any turn that touches **3+ files** via `Edit` / `Write` / `MultiEdit` MUST run `/qa-loop` before claiming done. `/qa-loop` iterates: dispatch `qa-agent` → fix found bugs → re-audit → repeat until `## VERIFICATION PASSED` or iteration cap hit. Build/typecheck alone is NOT sufficient — `/qa-loop` catches integration bugs, wiring issues, and logic errors that static checks miss. "I only changed small things in each file" is not an exception — scope is measured in files touched, not lines changed. If already inside a workflow command (`/ship`, `/autopilot`, `/tdd`, `/bug`) that has its own QA phase, that satisfies this rule.
