...
23. **Multi-file edits require QA, tiered by blast radius.** Any turn that touches **3+ files** via `Edit` / `Write` / `MultiEdit` MUST run a QA audit before claiming done. Build/typecheck alone is NOT sufficient. Default tier is the full `/qa-loop` (dispatch `qa-agent` → fix → re-audit until a CLEAN `## VERIFICATION PASSED` or cap; a pass marker whose body says `Assessment: PASS WITH CONCERNS` or `FAIL` is not clean, see `~/.claude/rules/agent-contracts.md` "qa-agent verdict mapping"). The light tier — a single `qa-agent` dispatch, no loop — is allowed ONLY when ALL hold: ≤150 changed lines (one recorded exception: a scaffold-only `repo-init` run, whatever its line count, Zalo 2026-09-23), behavior-preserving or narrowly additive, no auth/payment/data-deletion/migration paths, no new dependencies. State the tier chosen and why; when in doubt, full loop. If already inside a workflow command (`/autopilot`, `/bug`) that has its own QA phase, that satisfies this rule. (Tier basis: 2026-07 audit — full CLAUDE.md rule text has the details.)
