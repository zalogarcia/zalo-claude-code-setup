...
description: List all /autopilot worktrees + branches in the current repo with their terminal_state, commits ahead of a base, files touched, and task summary. Use when the user asks which autopilot runs finished, are still pending, or are left to merge, or as the discovery step inside /autopilot-merge. Returns a TSV table.
---
