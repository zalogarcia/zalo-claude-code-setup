---
name: repo-init
description: 'Scaffold a repo for Claude: scan the codebase, actually run the build/test/typecheck/lint commands, and generate .claude/CLAUDE.md, path-scoped .claude/rules/*.md, the machine-readable .claude/VERIFY.md verification manifest (deploy surfaces plus THE proof signal each deploy claim requires) and its runnable projection .claude/verify.sh. Use when the user asks to initialize or scaffold a repo for Claude, or when a repo lacks .claude/CLAUDE.md. Idempotent: fills gaps in existing scaffolds, never clobbers. Implements the global Project Init Protocol; ~/.claude/scripts/repo-drift-check.sh lists repos under ~/dev missing the scaffold.'
---
