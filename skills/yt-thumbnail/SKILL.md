---
name: yt-thumbnail
description: Generate Zalo Kabche YouTube thumbnails (1280×720) from the locked Shop Manual '74 spec using real reference photos of Zalo, via the image-craft-expert agent on the gpt-image-2 `images.edit` rail (OpenAI, the standing preference, NOT nano-banana), with a mandatory text + face-identity audit and the 120px squint test. Use when the user says "make the thumbnail", "thumbnail comps for video N", "thumbnail options", or at the packaging stage of any video in ~/dev/zalo-kabche-brand. Replaces one-line "make a thumbnail" prompts that garble text, drift the face, and violate the brand spec.
---
